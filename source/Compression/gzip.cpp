  ...
  do {
    ...
    if (ret != Z_OK && ret != Z_STREAM_END && ret != Z_BUF_ERROR) {
      ...
      throw std::runtime_error(
          "Decryption failed. Error during unzipping (inflate). In most "
          "cases this means your key does not match the database file.");
    }
...
void unzipBuffer(std::istream &compressed, std::ostream &uncompressed) {
  ...
  if (ret != Z_OK) {
    throw std::runtime_error("Decryption failed. Error during unzipping "
                             "(inflateInit). Invalid key?");
  }

  do {
    if (!compressed || compressed.eof()) {
      throw std::runtime_error(
          "Couldn't read compressed file (unexpected EOF).");
    }
