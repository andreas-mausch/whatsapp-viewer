...
PanelList<T, Panel>::PanelList(wxWindow *parent) {
  wxXmlResource::Get()->LoadObject(this, parent, _("PanelList"),
                                   "wxScrolledWindow");
}

template <class T, class Panel> PanelList<T, Panel>::~PanelList() { clear(); }

template <class T, class Panel> void PanelList<T, Panel>::clear() {
  GetSizer()->Clear(true);
}
